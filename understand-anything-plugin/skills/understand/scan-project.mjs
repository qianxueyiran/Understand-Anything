 ...
 *
 * Usage:
 *   node scan-project.mjs <projectRoot> <outputPath> [--scope-json <json-array>] [--require-scope] [--repository-output <path>]
 *
...
function parseArgs(argv) {
  const [, , projectRoot, outputPath, ...rest] = argv;
  let scopeJson = null;
  let requireScope = false;
  let repositoryOutputPath = null;
  for (let i = 0; i < rest.length; i++) {
    const arg = rest[i];
    if (arg === '--require-scope') {
      requireScope = true;
      continue;
    }
    if (arg === '--repository-output') {
      if (i + 1 >= rest.length) {
        throw new Error('--repository-output requires a path value');
      }
      repositoryOutputPath = rest[i + 1];
      i++;
      continue;
    }
    if (arg !== '--scope-json') {
    ...
    if (i + 1 >= rest.length) {
      throw new Error('--scope-json requires a JSON array value');
    }
    scopeJson = rest[i + 1];
    i++;
  }
  return { projectRoot, outputPath, scopeJson, requireScope, repositoryOutputPath };
}

function normalizeScopePath(raw) {
  if (typeof raw !== 'string') {
    throw new Error('--scope-json entries must be strings');
  }
  if (!raw.trim()) {
    throw new Error('--scope-json entries must not be empty');
  }
  if (isAbsolute(raw) || /^[A-Za-z]:[\\/]/.test(raw)) {
    throw new Error(`--scope-json entries must be project-relative: ${raw}`);
  }
  const normalized = posix.normalize(raw.replace(/\\/g, '/')).replace(/\/+$/, '');
  if (!normalized || normalized === '..' || normalized.startsWith('../')) {
    throw new Error(`--scope-json entry escapes project root: ${raw}`);
  }
...
function parseScopeJson(scopeJson) {
  ...
  try {
    ...
    throw new Error(`--scope-json must be valid JSON: ${err.message}`);
  }
  if (!Array.isArray(parsed)) {
    throw new Error('--scope-json must be a JSON array');
  }
  const seen = new Set();
  const scopes = [];
...
async function main() {
  const { projectRoot, outputPath, scopeJson, requireScope, repositoryOutputPath } = parseArgs(process.argv);
  if (!projectRoot || !outputPath) {
    process.stderr.write(
      'Usage: node scan-project.mjs <projectRoot> <outputPath> [--scope-json <json-array>] [--require-scope] [--repository-output <path>]\n',
    );
    process.exit(1);
  }
  const scopePaths = parseScopeJson(scopeJson);
  if (requireScope && scopePaths.length === 0) {
    throw new Error('--require-scope requires non-empty --scope-json');
  }
  const projectRootPath = resolve(projectRoot);

  if (!existsSync(projectRootPath)) {
    process.stderr.write(
      `scan-project.mjs failed: projectRoot does not exist: ${projectRootPath}\n`,
    );
    process.exit(1);
  }
  const projectRootStat = statSync(projectRootPath);
  if (!projectRootStat.isDirectory()) {
    process.stderr.write(
      `scan-project.mjs failed: projectRoot is not a directory: ${projectRootPath}\n`,
    );
    process.exit(1);
  }

  // 1. Enumerate. Either git ls-files or recursive walk.
  const candidates = enumerateFiles(projectRootPath);
  ...
  const combined = createIgnoreFilter(projectRootPath);
  const userIgnoresPresent = hasUserIgnoreFile(projectRootPath);
  ...
  for (const rel of kept) {
    const absPath = join(projectRootPath, rel);
  ...
  const scopedFileEntries = fileEntries.filter(file => isInScope(file.path, scopePaths));

  // 5. Stats.
  const byCategory = {};
  const byLanguage = {};
  for (const f of scopedFileEntries) {
    byCategory[f.fileCategory] = (byCategory[f.fileCategory] || 0) + 1;
    byLanguage[f.language] = (byLanguage[f.language] || 0) + 1;
  }

  const estimatedComplexity = estimateComplexity(scopedFileEntries.length);

  const output = {
    scriptCompleted: true,
    projectRoot: projectRootPath,
    files: scopedFileEntries,
    totalFiles: scopedFileEntries.length,
    filteredByIgnore,
    estimatedComplexity,
    stats: {
      filesScanned: scopedFileEntries.length,
      byCategory,
      byLanguage,
    },
  };
  ...
  if (repositoryOutputPath) {
    const repositoryOutput = {
      scriptCompleted: true,
      projectRoot: projectRootPath,
      repositoryFiles: fileEntries,
      totalFiles: fileEntries.length,
    };
    mkdirSync(dirname(repositoryOutputPath), { recursive: true });
    writeFileSync(repositoryOutputPath, JSON.stringify(repositoryOutput, null, 2), 'utf-8');
  }
  ...
  process.stderr.write(
    `scan-project: filesScanned=${scopedFileEntries.length} ` +
    `repositoryFiles=${fileEntries.length} ` +
