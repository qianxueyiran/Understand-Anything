 ...
 *
 * Usage:
 *   node extract-import-map.mjs <input.json> <output.json> [--repository-input <repository-files.json>]
 *
 * Input JSON:
 *   {
 *     projectRoot: <abs-path>,
 *     files: [{ path, language, fileCategory }, ...],
 *     repositoryFiles: [{ path, language, fileCategory }, ...] // optional
 *   }
 *
 * Output JSON:
 *   {
...
async function main() {
  ...
  let repositoryInputPath = null;
  for (let i = 0; i < rest.length; i++) {
    const arg = rest[i];
    if (arg !== '--repository-input') {
      throw new Error(`Unknown argument: ${arg}`);
    }
    if (i + 1 >= rest.length) {
      throw new Error('--repository-input requires a path value');
    }
    repositoryInputPath = rest[i + 1];
    i++;
  }
  if (!inputPath || !outputPath) {
    process.stderr.write('Usage: node extract-import-map.mjs <input.json> <output.json> [--repository-input <repository-files.json>]\n');
    process.exit(1);
  }
  ...
  if (repositoryInputPath) {
    repositoryInput = JSON.parse(readFileSync(repositoryInputPath, 'utf-8'));
  }
  const resolutionFiles = Array.isArray(repositoryInput?.repositoryFiles)
    ? repositoryInput.repositoryFiles
    : Array.isArray(input.repositoryFiles)
  ...
  if (!Array.isArray(resolutionFiles)) {
    throw new Error('Invalid input: repositoryFiles must be an array when present');
  }
  ...
  const ctx = buildResolutionContext(projectRoot, resolutionFiles);
