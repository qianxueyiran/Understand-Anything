...
- 进入 shard 模式后先运行：
  ```bash
  python "$PLUGIN_ROOT/skills/understand-domain/domain-shard-workflow.py" "$PROJECT_ROOT" --shard "$SHARD_ID" --prepare
  ```
  该脚本会机械校验 root manifest、shard id、code shard 文件存在性，并输出 `codeShardPath`、`intermediatePath`、`domainShardPath`、`manifestPath`。后续阶段必须使用这些路径，不要手写其它 shard 输出路径。
...
7. If `--shard <id>` was passed → run `python "$PLUGIN_ROOT/skills/understand-domain/domain-shard-workflow.py" "$PROJECT_ROOT" --shard "$SHARD_ID" --prepare`, keep its JSON output as `$DOMAIN_SHARD_PATHS`, then proceed to Phase 3 using only `$DOMAIN_SHARD_PATHS.codeShardPath`
...
1. Read `$PROJECT_ROOT/.understand-anything/knowledge-graph.json`; in shard mode, read only `$DOMAIN_SHARD_PATHS.codeShardPath`
...
4. In shard mode, the agent writes its output to `$DOMAIN_SHARD_PATHS.intermediatePath`
