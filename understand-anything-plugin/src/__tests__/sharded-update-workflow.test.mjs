...
describe("sharded update workflow commit", () => {
  ...
  it("commit rejects removed downstream flags", () => {
    const root = initRepo();

    for (const flag of ["--with-domain", "--with-product"]) {
      const result = spawnSync("node", [workflowScript, root, "commit", flag], { encoding: "utf-8" });
      expect(result.status).toBe(1);
      expect(result.stderr).toContain("--with-domain and --with-product were removed");
    }
  });
